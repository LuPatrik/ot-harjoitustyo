...
from enemy_ship import Enemy_ship
...
def main():
    ...
    enemy_ship = Enemy_ship(((screen_size[0]-52)+300)/2, 50, 1, 100)
    def refresh_screen():
    ...
    while running:
        ...
        for bullet in player_ship.bullets:
            if pygame.mask.from_surface(bullet.image).overlap(pygame.mask.from_surface(enemy_ship.image), (int(enemy_ship.x - bullet.x), int(enemy_ship.y - bullet.y))):
                enemy_ship.hit(10)
